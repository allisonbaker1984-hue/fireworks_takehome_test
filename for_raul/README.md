...
│   └── utils.py
└── data/
    ├── Chinook.db           (generated by setup.sh, not included in the zip)
    ├── dev_questions.json   (the 10 development questions this system was validated against)
    └── dev_answers.json     (this system's outputs: generated SQL and a human-readable answer for each question)
```
