...
- A Fireworks AI API key — sign up / grab a key at [fireworks.ai](https://fireworks.ai)
...
| `FIREWORKS_MODEL` | No | Overrides the default model (`accounts/fireworks/routers/kimi-k2p7-code-fast`). Only needed if you want to test a different Fireworks model. |
...
Connected. Model: accounts/fireworks/routers/kimi-k2p7-code-fast
...
- Ask follow-up questions in the same session ("now sort that by country") — conversation context carries forward.
...
- `dev_answers.json` — this system's outputs (generated SQL + human-readable answer) for each of those 10 questions
