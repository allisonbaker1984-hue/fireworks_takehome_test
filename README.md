...
│   └── utils.py
├── data/
│   ├── Chinook.db                    (generated by setup.sh, not committed)
│   ├── dev_questions.json
│   ├── dev_questions_with_answers.json
│   ├── dev_answers_example.json
│   └── dev_answers.json              (this system's outputs, see generate_dev_answers.py)
├── generate_dev_answers.py           (produces data/dev_answers.json)
├── compare_models.py                 (produces model_comparison.json)
├── diagnose_latency.py
├── llm_judge_eval.py                 (produces llm_judge_results.json)
├── model_comparison.json
├── llm_judge_results.json
├── email_to_raul.md
└── for_raul/                         (the actual customer-facing deliverable, see for_raul/README.md for its structure)
```
